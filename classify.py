# Predict the classes the testing files belong to
# using Gaussian Naive Bayes and Support Vector Machine
...
def mynormalize(dataarray,testdata):
	...
	result=np.zeros((h,w))
	#ignore nan. hiphop 36 37 gives nan result
	ave=np.nanmean(dataarray,axis=0)
	#ignore nan  
	var=np.nanvar(dataarray,axis=0)    
	for i in range(h):
		for j in range(w):
			if var[j]!=0:
...
# remove the chars in the data such as inf, nan
